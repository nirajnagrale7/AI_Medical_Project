...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        # ALWAYS show the extracted text for debugging
        with st.expander("🔍 Show Extracted Text (Debug Mode)", expanded=True):
        ...
        if extracted_text:
            ...
            if not analysis_results:
                st.warning("Could not find any standard medical parameters to analyze. The report might be in an unsupported format.")
            else:
                for key, result in analysis_results.items():
                    ...
                    if result['status'] == "Abnormal":
                        st.error(f"**Warning:** Your **{key.replace('_', ' ').title()}** level is outside the normal range.")
        else:
            st.error("Could not extract any text from the document. The file might be empty, corrupted, or a scan of poor quality.")

st.sidebar.header("About")
st.sidebar.info(
    "This AI assistant combines a symptom checker with a basic medical report analyzer. "
    "The report analyzer uses OCR and Regex to find common biomarkers like Hemoglobin, WBC count, etc."
)